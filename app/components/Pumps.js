...
class Pumps extends Component {
  ...
  handleChangeType(i, e) {
    ...
    let pump = this.state.data.slaves_modules.pump_module.pumps[i];
    pump.type = parseInt(e.target.value);
    //add new pump object to pumps
    this.setState({
      pumps: this.state.data.slaves_modules.pump_module.pumps.map(el =>
  ...
  handleSubmit(e) {
    ...
    console.log(
      'data will send : ' +
        JSON.stringify({
          pump_module: this.state.data.slaves_modules.pump_module
        })
    );
    ...
    return (
      <div className="w-100">
        {data.slaves_modules.pump_module.pumps.map((pump, index) => (
          ...
          >
            <div className="col-5 col-md-3 align-self-center alert alert-primary p-4 mb-0">
            ...
            <div
              onClick={event => this.handleClickState(index, event, pump.state)}
              className={
                pump.state
                  ? 'col-5 col-md-3 align-self-center alert alert-success text-center p-4 mb-0'
                  : 'col-5 col-md-3 align-self-center alert alert-secondary text-center p-4 mb-0'
              }
            >
              <button className="btn">
                <i className="fas fa-power-off" /> {pump.state ? 'On' : 'Off'}
              </button>
            </div>
            <div className="w-100" />
            <div className="col-10 col-md-6 alert alert-dark">
              <div className="row no-gutters justify-content-center">
                ...
                <input
                   ...
                   * 0 = 50000ms
                   * 1 = 5000ms provided by user through smartapp init @ 5000ms
                   * 2 = 5000ms provided by user through smartapp init @ 5000ms
                   */
                  max={
                    pump.type === 1
                      ? data.slaves_modules.water_sensor.ph.max_pumps_durations
                      : pump.type === 2
                      ? data.slaves_modules.water_sensor.ec.max_pumps_durations
                      : data.slaves_modules.water_sensor.ph.max_pumps_durations
                  }
                  step="10"
                  value={pump.duration}
                  onChange={event => this.handleChangeDuration(index, event)}
                />
